...
#https://www.rdocumentation.org/packages/dplyr/versions/1.0.10/topics/glimpse for reference
glimpse(surveys)
```
...
30,463 rows and 15 columns

**3. What are the data types of the variables in this dataset?**

<!-- You can either write code here or in the code chunk for #1, but I **must** see the code you used to address this question!  -->
record_id, month, day, year, plot_id, species_id, sex, hindfoot_length, weight, date, days_of_week, plot_type, genus, species, taxa
...
ggplot(data = surveys,
  ...
  #https://r-graph-gallery.com/294-basic-ridgeline-plot.html for ridgeplot reference
  geom_density_ridges() + 
  labs(x = "Weight", 
       y = "Species",
       title = "Rodents Comparision by Species", 
...
Because our P-Value of less than 2e-16 is much smaller than a significant level of 0.05, as well as a high F-Value of 19916, we reject the null hypothesis. There is statistically significant evidence that at least one rodent species has a different population mean weight.
